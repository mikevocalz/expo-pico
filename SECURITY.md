...
- The reflection probe (`PicoDevice.classPresent()`) relies on whatever SDK is on the classpath (PPS from Maven, or the legacy PVR AAR) being honest about its class names. A malicious fork could present the same class names and pass the probe; the plugin doesn't verify signatures.
