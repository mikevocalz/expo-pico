...
| --- | --- | --- |
| `account` | `getAccountLinkStatus()` | **`ISignInClient` has no account-linking call.** Nothing in the auth artifact returns a link status. This is a seam with no backing and should either be removed or documented as `NOT_IN_PPS_1_0` like rooms/rtc/storage. |
