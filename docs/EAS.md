...
| PPS Platform SDK calls (account, IAP, social, leaderboards, achievements, notifications, subscription, rooms, RTC, storage) return `SERVICE_UNAVAILABLE` at runtime | `picoDebug` was built with Gradle unable to reach `https://artifact.bytedance.com/repository/Volcengine/`, or the active flavor is `mobile`, or the host is not PICO hardware. | PPS deps (`com.pico.pps:platform-service-*:1.0.0`) resolve from public Maven automatically — confirm network access during prebuild, that you're on the `picoDebug` variant, and that you're running on a real PICO device. |
| Programmatic `setPassthrough()` or `expo-pico-spatial` anchors / scene mesh return `SERVICE_UNAVAILABLE` | These surfaces still depend on the **legacy** PICO Platform SDK 3.x / Spatial SDK 1.x (PVR-prefixed AARs), which are not on public Maven. | Download the AAR from the PICO Developer portal and drop it into `vendor/pico-sdk/` or `android/app/libs/`. Distinct from the modern PPS Maven artifacts. |
