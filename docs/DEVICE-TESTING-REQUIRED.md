...
### Example app — ReactVision/Viro renderer

The example's `PicoSceneRoot.tsx` / `GltfModel.tsx` use `@reactvision/react-viro`'s `<ViroVRSceneNavigator>` + `<Viro3DObject type="GLB">`. None of this is covered by unit tests — Viro can only run against its native module.

| Item | Test procedure | Risk if wrong |
| --- | --- | --- |
| `@reactvision/react-viro@2.56.0` autolinks correctly under Expo SDK 56 + RN 0.86 | Run `cd example && bun install && bun expo prebuild --clean`. Inspect `example/android/settings.gradle` — confirm `:viro_renderer` and `:viro_react_bridge` projects are included. If not, refresh `example/patches/` or pin the matching Expo / autolinking minor. | `<ViroVRSceneNavigator>` mounts but stays black; native bridge missing → no scene at all. |
| Viro's OpenXR session activates within ~1 s of scene mount | Launch the example on PICO Swan or Quest 3 / 3S. Scene tab should switch from the dim "loading…" HUD to an immersive scene rendering the BrainStem GLB. | Session never activates → headset shows the Horizon shell (Quest) or PICO launcher (PICO) instead of an immersive view. Almost always the `libopenxr_loader.so` declaration in the merged manifest is missing or `pvr.app.type` is wrong. |
| `Viro3DObject` loads `pico-demo.glb` (auto-downloaded by `scripts/download-demo-model.js`) | Scene HUD should read `scene: gltf loaded`. The BrainStem model should render through Viro's `GLB` loader with its first baked skeletal animation clip on loop. | Fallback: `scene: fallback primitive` + a procedural box. Common cause: GLB download was skipped (`EXPO_PICO_SKIP_DEMO_MODEL`) or `localUri` couldn't resolve through expo-asset. |
| Viro's OpenXR backend binds against `expo-pico-core`'s `<uses-native-library>` declaration | On PICO / Quest hardware with the app built at `xrMode: 'pico-swan'` (PICO) or `xrMode: 'pico-os5'` (Quest immersive build) + `appType: 'vr'`, `ViroVRSceneNavigator` should mount without `UnsatisfiedLinkError` on `libopenxr_loader.so`. The HUD's `xr session` row surfaces the outcome live. Expected on Swan / Quest: `requesting…` → `active`. On non-XR device: `unsupported`. | If the row stays at `requesting…` indefinitely or flips to `failed` with a native-library error, the Phase E `<uses-native-library>` declaration didn't land in the merged manifest — validate via `aapt dump xmltree`. |
...
| `getPicoDiagnostics()` produces correct findings on a real device | Install with intentional misconfig (e.g. `appType: '2d'` + `xrMode: 'pico-os5'`); confirm `appType.hidden-launcher` finding surfaces |
