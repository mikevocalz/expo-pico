...
| `xrMode` matches the hardware target.                                                  | `'pico-swan'` for Swan; `'pico-os5'` for PICO 4 / 4 Ultra / Neo3; `'mobile'` only for 2D companion builds.                   | Cross-reference ARCHITECTURE §15                                                                 |
...
| `minSdkVersion` matches the plugin default for your `xrMode`.            | 32 for `pico-os5`, 33 for `pico-swan` (or explicit override).               | inspect `android/app/build.gradle` after prebuild                                 |
